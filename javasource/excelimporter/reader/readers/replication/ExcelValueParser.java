...
public class ExcelValueParser extends AbstractValueExtractor {
	...
	private Object getValue(PrimitiveType type, String column, ExcelCellData cellData) throws ParseException {
		...
		Object formattedData = cellData.getFormattedData();
		Object rawData = cellData.getRawData();

		if (formattedData != null && formattedData.toString().equals(""))
		{
			return getValue(type, column, formattedData.toString());
		}

		if (settings.hasValueParser(column)) {
			return getValue(type, column, formattedData != null ? formattedData : rawData);
		}
		...
		switch (type) {
			...
			case Long:
				...
				if (!(parsed instanceof BigDecimal))
					throw new ParseException("Could not parse value '" + formattedData + "' to " + type.name() + " in column #" + (cellData.getColumnIndex() + 1));
				try {
			...
			default:
				try {
					...
					return getValue(type, column, rawData);
				}
		}
	}
