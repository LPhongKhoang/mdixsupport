...
public class ExcelImporter {
	...
	public static void getColumns(IContext context, IMendixObject template, IMendixObject excelDocument) throws CoreException {
		...
		final Long fileSize = getFileSize(context, excelDocument);
		if( fileSize == null || fileSize <= 0)
			throw new CoreException("You must select a valid file document before the columns can be extracted.");
	 ...
	 */
	public static long startImport(IContext context, IMendixObject template, IMendixObject templateDocument, IMendixObject parentObject) throws CoreException, ExcelImporterException {
		if (template == null)
			throw new CoreException("Template not found!");

		final String fileName = templateDocument.getValue(context, FileDocument.MemberNames.Name.toString());
		if (fileName == null || fileName.isBlank())
			throw new CoreException("No valid filepath found from template");

		final Long fileSize = getFileSize(context, templateDocument);
		if( fileSize == null || fileSize <= 0)
			throw new CoreException("You must select a valid file document before the data can be imported.");
	...
	private static long getFileSize( IContext context, IMendixObject fileDocument ) {
		return fileDocument.<Long>getValue(context, FileDocument.MemberNames.Size.toString());
	}
}
